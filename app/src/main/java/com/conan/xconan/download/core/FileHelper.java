...
import com.conan.xconan.download.mode.DownRange;
...
public class FileHelper {
    ...
    void saveFile(Subscriber<? super DownProgress> sub, File saveFile, Response<ResponseBody> resp) {
        ...
        try {
            try {
                ...
                XCoLog.i("Normal download completed!");
                sub.onCompleted();
            } finally {
            ...
            XCoLog.i("Normal download failed or cancel!");
            sub.onError(e);
        }
    }
    ...
    void saveFile(Subscriber<? super DownProgress> subscriber, int i, long start, long end,
        ...
        try {
            try {
                XCoLog.i(Thread.currentThread().getName() + " start download from " + start + " to " + end + "!");
                int readLen;
                byte[] buffer = new byte[8192];
                DownProgress status = new DownProgress();
                ...
                XCoLog.i(Thread.currentThread().getName() + " complete download! Download size is " +
            ...
            XCoLog.i(Thread.currentThread().getName() + " download failed or cancel!");
    ...
    private void createDirectories(String... directoryPaths) throws IOException {
        for (String each : directoryPaths) {
            ...
            if (file.exists() && file.isDirectory()) {
                XCoLog.d("Directory exists. Do not need create. Path = " + each);
            } else {
                boolean flag = file.mkdir();
                if (flag) {
                    XCoLog.d("Directory create succeed! Path = " + each);
                } else {
                    XCoLog.d("Directory create failed! Path = " + each);
