package com.conan.xconan.log.inner;
...
public abstract class Tree implements ITree {
    ...
    private StackTraceElement getCurrentStackTrace() {
        ...
        int stackOffset = getStackOffset(trace, XCoLog.class);
        if (stackOffset == -1) {
            return null;
        }
    ...
    private int getStackOffset(StackTraceElement[] trace, Class cla) {
        for (int i = LogConstant.MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (cla.equals(XCoLog.class) && i < trace.length - 1 && trace[i + 1].getClassName()
                    .equals(XCoLog.class.getName())) {
                continue;
            }
